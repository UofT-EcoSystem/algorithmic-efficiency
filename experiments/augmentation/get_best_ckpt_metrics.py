...
for i in range(1, num_trials + 1):
  ...
  trial_best = pd.concat([trial_best, df.iloc[[best]][['loss', 'accuracy', 'epoch', 'global_step']]])
...
for i in range(num_trials):
  print('Trial%i - step=%i, epoch=%i' % (i, trial_best['global_step'].values[i], trial_best['epoch'].values[i]))
